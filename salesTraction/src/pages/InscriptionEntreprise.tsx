...
function InscriptionEntreprise() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (siret.length !== 14) {
      setError(t("registration.startup.siretError"));
      return;
    }
    ...
    try {
      ...
      console.log(t('registration.startup.registrationSuccess'), result.startup);
        ...
        <div className="authComponent_container w-full bg-white shadow-md rounded-lg p-6 mt-8">
            <div className='authComponent_content'>
                  <form onSubmit={handleSubmit}>
                    <label className="labels">{t("registration.startup.email")}</label>
                    ...
                    <label className="labels">{t("registration.startup.siretNumber")}</label>
                    ...
                    <label className="labels">{t("registration.startup.companyName")}</label>
                    ...
                    <label className="labels">{t("registration.startup.sectors")}</label>
                    ...
                    <label className="labels">{t("registration.startup.description")}</label>
                    ...
                    <label className="labels">{t("registration.startup.password")}</label>
                    ...
                    <div className='auth_buttons flex flex-col md:flex-row justify-between gap-4'>
                        <input type='submit' value={t("registration.startup.register")} className='auth_button w-full md:w-1/2 py-2 bg-blue-500 text-white rounded'/>
                        <input type='button' value={t("registration.startup.cancel")} className='auth_button w-full md:w-1/2 py-2 bg-gray-500 text-white rounded' onClick={handleRetour}/>
                    </div>
                  </form>
