  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(t('errors.failedToFetch')+`: ${response.statusText}`);
        }
        ...
        setError(t('errors.fetchProfile')+`: ${err instanceof Error ? err.message : 'Unknown error'}`);
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg max-w-md mx-auto">
          <h3 className="font-bold text-lg mb-2">{t('errors.title')}</h3>
          <p>{error}</p>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 p-4 rounded-lg max-w-md mx-auto">
          <h3 className="font-bold text-lg mb-2">{t('states.profilNotFound.title')}</h3>
          <p>{t('states.profilNotFound.message')}</p>
        </div>
      </div>
    );
  }
  ...
  if (userType === 'student') {
    ...
    return (<>
      ...
      <div className="student-profile mx-auto px-4 py-6 max-w-4xl">
        ...
        <h1 className="text-xl sm:text-2xl font-bold mb-4 text-center sm:text-left">{t('profile.student.title')}</h1>
        ...
        <div className="profile-details grid grid-cols-1 gap-4 sm:gap-6">
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              ...
              {t('profile.student.sections.contactInfo.title')}
            </h3>
            <p className="my-2 text-sm sm:text-base"><strong> {t('profile.student.sections.contactInfo.email')}:</strong> {studentData.user.email ||  t('profile.student.sections.contactInfo.noEmail')}</p>
            {studentData.linkedin_url && (
              <p className="my-2 text-sm sm:text-base overflow-hidden text-ellipsis">
                <strong> {t('profile.student.sections.contactInfo.linkedin')}:</strong> 
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
               ...
               {t('profile.student.sections.availabilityPeriod.title')}
            </h3>
            <div className="flex flex-col sm:flex-row sm:justify-between">
              <p className="my-2 text-sm sm:text-base"><strong>{t('profile.student.sections.availabilityPeriod.from')}:</strong> {formatDate(studentData.starting_date)}</p>
              <p className="my-2 text-sm sm:text-base"><strong>{t('profile.student.sections.availabilityPeriod.to')}:</strong> {formatDate(studentData.ending_date)}</p>
            </div>
          </div>
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              ...
              {t('profile.student.sections.personalInfo.title')}
            </h3>
            <p className="my-2 text-sm sm:text-base"><strong>{t('profile.student.sections.personalInfo.birthDate')}:</strong> {formatDate(studentData.birth_date)}</p>
          </div>
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              ...
              {t('profile.student.sections.languages.title')}
            </h3>
            <div className="flex flex-wrap gap-2">
              {studentData.languages && studentData.languages.length > 0 ? (
                studentData.languages.map((item, index) => (
                  ...
                  >
                    {item.language || item.languageSpoken?.language || t('profile.student.sections.languages.unknownLanguage')}
                  </span>
                ))
              ) : (
                <p className="text-sm sm:text-base text-gray-500">{t('profile.student.sections.languages.noLanguage')}</p>
              )}
            </div>
          </div>
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              ...
              {t('profile.student.sections.sectorPreferences.title')}
            </h3>
            <div className="flex flex-wrap gap-2">
              {studentData.sectorPreferences && studentData.sectorPreferences.length > 0 ? (
                studentData.sectorPreferences.map((item, index) => (
                  ...
                  >
                    {item.sector_preference || item.sectorPreference?.sector_preference || t('profile.student.sections.sectorPreferences.unknownSector')}
                  </span>
                ))
              ) : (
                <p className="text-sm sm:text-base text-gray-500">{t('profile.student.sections.sectorPreferences.noSectorPreferences')}</p>
              )}
            </div>
          </div>
        </div>
  ...
  // Render enterprise profile
    ...
    return (
      ...
      <div className="enterprise-profile mx-auto px-4 py-6 max-w-4xl">
        <h1 className="text-xl sm:text-2xl font-bold mb-4 text-center sm:text-left">{t('profile.enterprise.title')}</h1>
        ...
        <div className="profile-header mb-6 bg-white p-4 rounded-lg shadow text-center sm:text-left">
          <div className="profile-name">
            ...
            <p className="text-gray-600">{t('profile.enterprise.sections.sector')}: {enterpriseData.secteur}</p>
          </div>
        </div>
        ...
        <div className="profile-details grid grid-cols-1 gap-4 sm:gap-6">
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              ...
              {t('profile.enterprise.sections.businessInfo.title')}
            </h3>
            <p className="my-2 text-sm sm:text-base"><strong>{t('profile.enterprise.sections.businessInfo.siret')}:</strong> {enterpriseData.siret}</p>
            <p className="my-2 text-sm sm:text-base"><strong>{t('profile.enterprise.sections.businessInfo.email')}:</strong> {enterpriseData.user?.email || t('profile.enterprise.sections.businessInfo.noEmail')}</p>
          </div>
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              ...
              {t('profile.enterprise.sections.description.title')}
            </h3>
            <p className="my-2 text-sm sm:text-base">{enterpriseData.description || t('profile.enterprise.sections.description.noDescription')}</p>
          </div>
          ...
          <div className="detail-item bg-white p-4 rounded-lg shadow">
            <h3 className="text-md sm:text-lg font-medium mb-2 border-b pb-2 flex items-center">
              ...
              {t('profile.enterprise.sections.status.title')}
            </h3>
            <p className={`my-2 text-sm sm:text-base py-1 px-3 rounded-full inline-flex items-center ${enterpriseData.is_validated ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}>
              {enterpriseData.is_validated 
                ? t('profile.enterprise.sections.status.validated')
                : t('profile.enterprise.sections.status.pendingValidation') }
            </p>
          </div>
        </div>
      </div></>
